...
void ModuleModelo::LogInfo(const std::string& pMessage)
{
...
void ModuleModelo::LogError(const std::string& pMessage)
{
	Assimp::DefaultLogger::get()->error(pMessage);
}

void ModuleModelo::LoadModel(const char* path)
{
	meshes.clear();
	modelPath = "Models/";
...
void ModuleModelo::processNode(aiNode* node, const aiScene* scene)
{
...
Mesh ModuleModelo::processMesh(aiMesh* mesh, const aiScene* scene)
{
	Mesh loadedMesh;
...
std::vector<Texture> ModuleModelo::loadTextures(aiMaterial* mat, aiTextureType type, char* typeName)
{
...
void ModuleModelo::UpdateTexture(Texture& textToUpdate)
{
...
bool ModuleModelo::FileExists(const char* path)
{
	if (access(path, 0) == -1)
	{
